...
model = init_chat_model(model="solar-pro", model_provider="upstage", temperature=1.0)
...
def write_story(state: WriteState):
    ...
    msg = model.invoke(f"'{topic}'를 주제로 짧고 감동적인 소설을 작성해 줘")
    return {"story": msg.content}


def write_joke(state: WriteState):
    topic = state["topic"]
    print(f"[worker C] 조크(joke)작성 시작")
...
if __name__ == "__main__":
    ...
    inputs = {"topic": "어느 70대 중반의 봄의 의미는?"}
